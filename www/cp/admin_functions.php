...
if (!isset($mad_install_active)) {
    $mad_install_active = 0;
}
/*Main Functions*/
global $today_day;
$today_day = date('d', mktime(0, 0, 0, date("m"), date("d"), date("Y")));
global $today_month;
$today_month = date('m', mktime(0, 0, 0, date("m"), date("d"), date("Y")));
global $today_year;
$today_year = date('Y', mktime(0, 0, 0, date("m"), date("d"), date("Y")));
...
function graph_report_widget($location, $type, $duration)
{
    ...
    echo '<td>&nbsp;</td>';
    echo '<th>';
    echo date('d/M/Y', mktime(0, 0, 0, date("m"), date("d") - 6, date("Y")));
    echo '</th>';
    echo '<th>';
    echo date('d/M/Y', mktime(0, 0, 0, date("m"), date("d") - 5, date("Y")));
    echo '</th>';
    echo '<th>';
    echo date('d/M/Y', mktime(0, 0, 0, date("m"), date("d") - 4, date("Y")));
    echo '</th>';
    echo '<th>';
...
//$data=sanitize($data);
    ...
    if ($datatype == 'profilesettings') {
        ...
        if (!isset($data['tooltip_setting'])) {
            $data['tooltip_setting'] = 0;
        }
    ...
    if ($datatype == 'generalsettings') {
        global $maindb;
        if (!isset($data['allow_statistical_info'])) {
            $data['allow_statistical_info'] = '';
        }
...
function get_publications()
{
    ...
    while ($publication_detail = mysql_fetch_array($usrres)) {
        ...
        if (!is_numeric($publication_detail['md_lastrequest'])) {
            $publication_status = 'No Ad-Requests yet';
        } else if (($current_timestamp - $publication_detail['md_lastrequest']) < MAD_PUBL_ACTIVE_CRITERIA_SECONDS) {
...
function get_placements($pubid)
{
    ...
    while ($zone_detail = mysql_fetch_array($usrres)) {
        ...
        if ($zone_detail['zone_type'] == "banner") {
            $zone_type = 'Standard Banner';
            $zone_size = '' . $zone_detail['zone_width'] . 'x' . $zone_detail['zone_height'] . '';
        }
        if ($zone_detail['zone_type'] == "interstitial") {
            $zone_type = 'Interstitial';
            $zone_size = 'Full Page';
        }

        if (!is_numeric($zone_detail['zone_channel'])) {
            $zone_channel = 'Default';
        } else {
        ...
        if (!is_numeric($zone_detail['zone_lastrequest'])) {
            $zone_status = 'No Ad-Requests';
        } else if (($current_timestamp - $zone_detail['zone_lastrequest']) < MAD_PUBL_ACTIVE_CRITERIA_SECONDS) {
            $zone_status = 'Active';
        } else {
...
function create_rightset($type, $id, $data)
{
    ...
    if (!isset($data['view_own_campaigns'])) {
        $data['view_own_campaigns'] = '';
    }
    if (!isset($data['view_all_campaigns'])) {
        $data['view_all_campaigns'] = '';
    }
    if (!isset($data['create_campaigns'])) {
        $data['create_campaigns'] = '';
    }
    if (!isset($data['view_publications'])) {
        $data['view_publications'] = '';
    }
    if (!isset($data['modify_publications'])) {
        $data['modify_publications'] = '';
    }
    if (!isset($data['view_advertisers'])) {
        $data['view_advertisers'] = '';
    }
    if (!isset($data['modify_advertisers'])) {
        $data['modify_advertisers'] = '';
    }
    if (!isset($data['ad_networks'])) {
        $data['ad_networks'] = '';
    }
    if (!isset($data['campaign_reporting'])) {
        $data['campaign_reporting'] = '';
    }
    if (!isset($data['own_campaign_reporting'])) {
        $data['own_campaign_reporting'] = '';
    }
    if (!isset($data['publication_reporting'])) {
        $data['publication_reporting'] = '';
    }
    if (!isset($data['network_reporting'])) {
        $data['network_reporting'] = '';
    }
    if (!isset($data['configuration'])) {
        $data['configuration'] = '';
    }
    if (!isset($data['traffic_requests'])) {
        $data['traffic_requests'] = '';
    }
...
function get_code_snippets($type, $zoneid)
{
    ...
    while ($snippet_detail = mysql_fetch_array($usrres)) {
        ...
        if ($type == 'tabcontent') {
            ...
            $data = file_get_contents(MAD_PATH . '/www/cp/templates/code_snippets/' . $snippet_detail['snippet_file'] . '');
            $data = str_replace("REPL_MADZONEHASH", $zone_detail['zone_hash'], $data);
            $data = str_replace("REPL_MADURL", $md_baseurl, $data);
            echo $data;
            echo '</textarea>';
            echo '</div> <!-- .widget-content -->';
        }
...
function get_campaigns()
{
    ...
    while ($campaign_detail = mysql_fetch_array($usrres)) {
        ...
        if ($campaign_detail['campaign_priority'] == 1) {
            $campaign_priority = '1';
        } else if ($campaign_detail['campaign_priority'] == 2) {
            $campaign_priority = '2';
        } else if ($campaign_detail['campaign_priority'] == 3) {
            $campaign_priority = '3';
        } else if ($campaign_detail['campaign_priority'] == 4) {
            $campaign_priority = '4';
        } else if ($campaign_detail['campaign_priority'] == 5) {
            $campaign_priority = '5';
        }

        if ($campaign_detail['campaign_type'] == 1) {
            $campaign_type = 'Direct Sold';
        } else if ($campaign_detail['campaign_type'] == 2) {
            $campaign_type = 'Promotional';
        } else if ($campaign_detail['campaign_type'] == 'network') {
        ...
        if ($campaign_detail['campaign_type'] != 'network' && $total_adunits < 1) {
            ...
            $statuscolor = '#000000';
        } else if ($campaign_detail['campaign_status'] == 2) {
            $campaign_status = 'Paused';
            $statuscolor = '#FF9900';
        } else if ($campaign_detail['campaign_start'] > date("Y-m-d")) {
            $campaign_status = 'Not Started';
            $statuscolor = '#000000';
        } else if ($campaign_detail['campaign_end'] <= date("Y-m-d")) {
            $campaign_status = 'Ended';
            $statuscolor = '#993300';
        } else if ($campaign_detail['campaign_status'] == 1) {
...
function json_regions($selected, $functionid)
{
    ...
    while ($region_detail = mysql_fetch_array($usrres)) {
        if ($region_detail['targeting_type'] == 'REGION') {
            $region_appendix = ' (Region)';
        } else {
            $region_appendix = '';
        }
        if ($x == 1) {
...
function list_placements_campaign($selected)
{
    ...
    while ($publication_detail = mysql_fetch_array($usrres)) {
        ...
        while ($zone_detail = mysql_fetch_array($zoneres)) {
            ...
            if (in_array($zone_detail['entry_id'], $selected)) {
...
function list_channels_campaign($selected)
{
    ...
    while ($channel_detail = mysql_fetch_array($usrres)) {
        ...
        echo '<td width="45%"><div>';
        if (in_array($channel_detail['channel_id'], $selected)) {
            $selectedhtml = 'checked="checked"';
        } else {
            $selectedhtml = '';
        }
...
function get_adnetworks($origin)
{
    ...
    while ($network_detail = mysql_fetch_array($usrres)) {
        ...
        if ($network_detail['interstitial_support'] == 1) {
            $interstitial_support_icon = 'icon-check';
        } else {
            $interstitial_support_icon = 'icon-x';
        }
        if ($network_detail['banner_support'] == 1) {
            $banner_support_icon = 'icon-check';
        } else {
            $banner_support_icon = 'icon-x';
        }
...
function get_adunits($campaignid)
{
    ...
    while ($ad_detail = mysql_fetch_array($usrres)) {
        ...
        if ($ad_detail['adv_status'] == 2) {
            $ad_status = 'Paused';
            $statuscolor = '#FF9900';
        } else if ($ad_detail['adv_status'] == 1) {
            $ad_status = 'Active';
            $statuscolor = '#009900;';
        }
...
function update_publisher_ids($data, $networkid, $publication_id)
{
    ...
    if (!isset($data['default-1'])) {
        $data['default-1'] = '';
    }
    if (!isset($data['default-2'])) {
        $data['default-2'] = '';
    }
    if (!isset($data['default-3'])) {
        $data['default-3'] = '';
    }
    if (!isset($data['default-4'])) {
        $data['default-4'] = '';
    }
    ...
    if (!is_numeric($publication_id)) {
        ...
        while ($publication_detail = mysql_fetch_array($publication_res)) {
            ...
            if (!isset($data['' . $publication_detail['inv_id'] . '-1'])) {
                $data['' . $publication_detail['inv_id'] . '-1'] = '';
            }
            if (!isset($data['' . $publication_detail['inv_id'] . '-2'])) {
                $data['' . $publication_detail['inv_id'] . '-2'] = '';
            }
            if (!isset($data['' . $publication_detail['inv_id'] . '-3'])) {
                $data['' . $publication_detail['inv_id'] . '-3'] = '';
            }
            if (!isset($data['' . $publication_detail['inv_id'] . '-4'])) {
                $data['' . $publication_detail['inv_id'] . '-4'] = '';
            }
        ...
        while ($zone_detail = mysql_fetch_array($publication_res)) {
            ...
            if (!isset($data['' . $zone_detail['entry_id'] . '-1'])) {
                $data['' . $zone_detail['entry_id'] . '-1'] = '';
            }
            if (!isset($data['' . $zone_detail['entry_id'] . '-2'])) {
                $data['' . $zone_detail['entry_id'] . '-2'] = '';
            }
            if (!isset($data['' . $zone_detail['entry_id'] . '-3'])) {
                $data['' . $zone_detail['entry_id'] . '-3'] = '';
            }
            if (!isset($data['' . $zone_detail['entry_id'] . '-4'])) {
                $data['' . $zone_detail['entry_id'] . '-4'] = '';
            }
