...
class SMAATO implements networkmodule
{
    public function request($request_type, $request_info, $network_ids, $backfill)
    {
        ...
        if ($request_type == 'banner' or $request_type == 'interstitial') {
            ...
            if (isset($_GET['o'])) {
                $request_url = $request_url . '&ownid=' . $_GET['o'];
            }
        ...
        if (isset($xml_response->ads->ad->link)) {
            $tempad['image'] = $xml_response->ads->ad->link;
        } else {
            $tempad['image'] = '';
        }
        if (isset($xml_response->ads->ad->mediadata)) {
            $tempad['markup'] = $xml_response->ads->ad->mediadata;
        } else {
            $tempad['markup'] = '';
        }
        if (isset($xml_response->ads->ad->adtext)) {
            $tempad['text'] = $xml_response->ads->ad->adtext;
        } else {
            $tempad['text'] = '';
        }
        if (isset($xml_response->ads->ad->action['target'])) {
            $tempad['url'] = $xml_response->ads->ad->action['target'];
        } else {
            $tempad['url'] = '';
        }
        ...
        if (isset($xml_response->ads->ad->beacons->beacon)) {
            $tempad['pixel'] = $xml_response->ads->ad->beacons->beacon;
        } else {
            $tempad['pixel'] = '';
        }
